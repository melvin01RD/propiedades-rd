...
> **Nota sobre `alembic check`:** Este proyecto no usa `alembic check` como paso de release
> porque el schema inicial (`001_initial_schema.sql`) fue aplicado manualmente antes de
> instalar Alembic. Alembic detectaría como "sobrantes" la tabla `spatial_ref_sys` (gestionada
> por PostGIS) y los índices del schema manual — ambos excluidos explícitamente en `env.py`
> mediante `include_object`. La DB y los modelos están sincronizados correctamente.
